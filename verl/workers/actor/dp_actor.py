...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        # Include uid for shifted GRPO gradient offset
        ...
        print(f"[DEBUG] num_mini_batches={len(mini_batches)}, ppo_epochs={self.config.ppo_epochs}, batch_size={len(data.batch)}")
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # Pass index for shifted GRPO if available
                    policy_loss_kwargs = {
                    ...
                    if "uid" in model_inputs:
                        policy_loss_kwargs["index"] = model_inputs["uid"]
                    
                    # Pass length_filter_keep_fraction if available from meta_info
                    if "length_filter_keep_fraction" in micro_batch.meta_info:
                        policy_loss_kwargs["length_filter_keep_fraction"] = micro_batch.meta_info["length_filter_keep_fraction"]
                    
                    # Call policy loss function and handle variable return values
                    loss_outputs = policy_loss_fn(**policy_loss_kwargs)
                    pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = loss_outputs[:4]
                    # Extract mean_ratio if returned (5th value for shifted_grpo1)
                    ratio = loss_outputs[4] if len(loss_outputs) > 4 else None
                    ...
                    # Log importance ratio if returned from policy loss
                    if ratio is not None:
                        micro_batch_metrics["importance_ratio/mean"] = ratio.mean().detach().item()
                        micro_batch_metrics["importance_ratio/max"] = ratio.max().detach().item()
                        micro_batch_metrics["importance_ratio/min"] = ratio.min().detach().item()
