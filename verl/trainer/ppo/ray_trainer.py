...
class RayPPOTrainer:
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler: Optional[Sampler]):
        ...
        # Skip train dataloader assertion in val_only mode
        ...
        total_training_steps = len(self.train_dataloader) * self.config.trainer.total_epochs if len(self.train_dataloader) > 0 else 0
    ...
    def _filter_rollout_by_length(self, batch: DataProto, keep_fraction: float, metrics: dict) -> DataProto:
        ...
        metrics["rollout/batch_size/original"] = original_size
        metrics["rollout/batch_size/filtered"] = filtered_size
        metrics["rollout/length/original"] = response_lengths.mean().item()
        metrics["rollout/length/filtered"] = response_lengths[indices_to_keep].mean().item()
        
        return filtered_batch

    def _filter_rollout_by_r_var(self, batch: DataProto, keep_fraction: float, metrics: dict) -> DataProto:
        """Filter rollout samples to maximize outcome reward variance within each prompt group.

        For each unique prompt (identified by uid), this method keeps the samples that have
        the largest deviation from the group mean reward, thereby maximizing the variance
        of the remaining samples. This helps GRPO/RLOO style algorithms by keeping more
        informative contrastive samples.

        Args:
            batch (DataProto): The batch containing rollout data with repeated responses
            keep_fraction (float): Fraction of samples to keep (e.g., 0.5 keeps top 50% by deviation)
            metrics (dict): Dictionary to update with filtering metrics

        Returns:
            DataProto: Filtered batch containing samples with highest reward deviation
        """
        # Get outcome rewards - use token_level_rewards if available, otherwise token_level_scores
        if "token_level_rewards" in batch.batch:
            token_rewards = batch.batch["token_level_rewards"]
        else:
            token_rewards = batch.batch["token_level_scores"]
        
        # Get response mask to properly sum only response tokens
        response_mask = batch.batch["response_mask"]
        uid = batch.non_tensor_batch["uid"]
        
        # Compute outcome reward for each sample (sum of token-level rewards over response)
        # token_rewards shape: (batch_size, seq_len) or similar
        outcome_rewards = (token_rewards * response_mask).sum(dim=-1)  # (batch_size,)
        
        # Group samples by uid (original prompt)
        unique_uids = np.unique(uid)
        indices_to_keep = []
        
        total_original_variance = 0.0
        total_filtered_variance = 0.0
        ...
        for unique_uid in unique_uids:
            ...
            # Get outcome rewards for this group
            group_rewards = outcome_rewards[uid_indices]
            
            # Compute mean reward for this group
            group_mean = group_rewards.mean()
            
            # Compute absolute deviation from mean
            deviations = torch.abs(group_rewards - group_mean)
            
            # Sort by deviation (descending) and keep top fraction with highest deviation
            n_keep = max(1, int(len(uid_indices) * keep_fraction))
            sorted_indices = torch.argsort(deviations, descending=True)[:n_keep]
            ...
            if len(group_rewards) > 1:
                total_original_variance += group_rewards.var().item()
            if n_keep > 1:
                kept_rewards = group_rewards[sorted_indices]
                total_filtered_variance += kept_rewards.var().item()
        ...
        metrics["rollout/batch_size/original"] = original_size
        metrics["rollout/batch_size/filtered"] = filtered_size
        metrics["rollout/outcome_reward_variance/original"] = total_original_variance / n_groups if n_groups > 0 else 0.0
        metrics["rollout/outcome_reward_variance/filtered"] = total_filtered_variance / n_groups if n_groups > 0 else 0.0
    ...
    def _validate(self):
        ...
        # Remove keys with mismatched lengths to avoid index errors downstream
        keys_to_remove = []
        for key_info, lst in reward_extra_infos_dict.items():
            # Allow reward_extra_info to have fewer entries than sample_scores if the reward function
            # only provides info for a subset of samples (e.g., unique prompts)
            if len(lst) > 0 and len(lst) != len(sample_scores):
                print(
                    f"[WARNING] Reward extra info key '{key_info}' has {len(lst)} entries but "
                    f"sample_scores has {len(sample_scores)} entries. "
                    f"This may occur when reward function returns info per unique prompt rather than per sample. "
                    f"Removing this extra info to avoid issues downstream."
                )
                # Mark for removal to avoid issues downstream
                keys_to_remove.append(key_info)
        
        # Remove mismatched keys
        for key in keys_to_remove:
            del reward_extra_infos_dict[key]
        ...
        for data_source, var2metric2val in data_src2var2metric2val.items():
            ...
            for var_name, metric2val in var2metric2val.items():
                ...
                for metric_name, metric_val in metric2val.items():
                    if (
                        (var_name == core_var)
                        and any(metric_name.startswith(pfx) for pfx in ["mean", "pass"])
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("old_log_prob", timing_raw, color="blue"):
                        ...
                        # Store entropy for advantage estimators that need it (e.g., GRPO_NSR_FIX)
                        # Must be numpy array (replicated for each batch sample) to satisfy chunking requirements
                        batch.non_tensor_batch["current_entropy"] = np.full(len(batch.batch), entropy_agg.detach().item())
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with marked_timer("update_actor", timing_raw, color="red"):
                            ...
                            batch.meta_info["length_filter_keep_fraction"] = self.config.actor_rollout_ref.rollout.get(
                                "length_filter_keep_fraction", 1.0
                            )

                            # compute the loss and update the actor
